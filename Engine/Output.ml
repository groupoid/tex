...
let output_pages job pages =
  match pages with
    [] -> ()
  | _ ->
      let comment =
        " ant output " ^ string_of_int (job.time.Unix.tm_year + 1900) ^ "." ^
      ...
      | PDF -> Runtime.GeneratePDF.write_pdf_file job.output_file comment pages
      | PS ->
          GeneratePostScript.write_postscript_file job.output_file comment
            pages
