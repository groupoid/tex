...
let lookup_in_dict dict name conv default = match DynUCTrie.lookup_string name dict with
...
let check_keys dict key_dict loc = begin
  DynUCTrie.iter
    (fun k _ ->
...
let parse_line_param loc dict =
  ...
  | (None,   Some w) -> Some (simple_cw num_zero w) in
  (baseline_skip, line_skip_limit, line_skip, leading, club_widow_penalty)

let parse_par_shape left_stream right_stream = begin
  ...
  let rec parse_shape stream = begin
    Parser.skip_spaces stream;
    if UCStream.next_char stream >= 0 then begin
...
let parse_boxes ps code env = begin
...
let parse_annotation ps left_annotation right_annotation = begin
  let new_ps = duplicate ps in
  let annotate env boxes = begin
...
let footnote_area_dict =
  ...
  (DynUCTrie.add_string str_math_params       ()
    DynUCTrie.empty)))))))))
let parse_footnote_area_dict ps loc dict = begin
...
let set_param ps = begin
  let param = arg_expanded ps in
  let loc   = location ps in
  let val_   = arg_key_val  ps in
  let add_cmd cmd parse = add_node ps (Node.Command (loc, cmd (parse loc val_))) in
...
let set_mark ps = begin
  let mark = Array.of_list (arg_expanded ps) in
  let val_  = Array.of_list (arg_expanded ps) in
  add_node ps (Node.CommandBox (location ps, `PageCmd (Box.SetMark (mark, val_))))
end
(* boxes and glue *)
...
let kern ps = begin
  ...
  | `Paragraph | `HBox      -> add_node ps (Node.Glue (location ps, skip, (fun _ -> dim_zero), true, true))
  | `Math                   -> Types.runtime_error "kerns in mathmode are not supported yet!"
  | `LRBox | `RLBox          -> add_node ps (Node.Glue (location ps, skip, (fun _ -> dim_zero), true, true))
  | _                       -> ()
end
let hbox dir ps = begin
  ...
  | `Default -> `HBox
  in
  let open_hbox close_command = begin
    if CharCode.cat_code (UCStream.next_char ps.input_stream) = CharCode.BeginGroup then begin
(*      Mode.ensure_par_mode ps; *)
  ...
  if Parser.match_substring ps.input_stream [116; 111] 0 then begin       (* to *)
  ...
  else if Parser.match_substring ps.input_stream [115; 112; 114; 101; 97; 100] 0 then begin  (* spread *)
...
let vbox ps = begin
  ...
  let open_vbox close_command = begin
  ...
  Parser.skip_blanks ps.input_stream;
  if Parser.match_substring ps.input_stream [116; 111] 0 then begin
    UCStream.remove ps.input_stream 2;
    Parser.skip_spaces ps.input_stream;
    let height = Parser.read_simple_skip_expression ps.input_stream in
    open_vbox (fun c -> close_vbox_to height c)
  end
  else if Parser.match_substring ps.input_stream [115; 112; 114; 101; 97; 100] 0 then begin
    UCStream.remove ps.input_stream 6;
    Parser.skip_spaces ps.input_stream;
    let amount = Parser.read_simple_skip_expression ps.input_stream in
  ...
  let get_scale options dpi = begin
    if dpi >/ num_zero then
      ...
      | Some (Some str) -> Parser.str_expr_to_num str // num_of_int 100
                           (* LoadImage assumed a default dpi let of 100. *)
      | _ -> begin
          log_warn (location ps) "No dpi let specified!";
          num_one
        end
    end
  end in
(*  let get_dpi options info = begin
...
(*  | [ 99] -> add_node ps (Node.PutGalleyInVBox (location ps, true, name)); *)
  | [116] -> add_node ps (Node.PutGalleyInVBox (location ps, true, name))
  | _     -> add_node ps (Node.PutGalleyInVBox (location ps, false, name))
  end;
...
let begin_table_entry ps = begin
...
let new_table_entry ps = begin
  end_table_entry   ps;
  begin_table_entry ps
end
let new_table_row ps = begin
  end_table_entry ps;
  let bl = get_counter ps str_table_entry_baseline in
...
let save_command ps = begin
  let name = Parser.read_argument ps.input_stream in
  ParseState.save_command ps name
end
let restore_command ps = begin
  let name = Parser.read_argument ps.input_stream in
  ParseState.restore_command ps name
end
let save_pattern ps = begin
  let name = Parser.read_argument ps.input_stream in
  ParseState.save_pattern ps name
end
let restore_pattern ps = begin
  let name = Parser.read_argument ps.input_stream in
  ParseState.restore_pattern ps name
end
let define_environment ps = begin
  let name       = Parser.read_argument ps.input_stream in
  let tmpl       = opt_expanded ps [] in
  let args       = Macro.parse_arg_template
                     (location ps)
...
let begin_environment ps = begin
  Macro.begin_env ps (Parser.read_argument ps.input_stream)
end
let end_environment ps = begin
...
let toggle_math ps = match current_mode ps with
| `Math -> ignore (Mode.set_mode ps `Paragraph)
| _     -> ignore (Mode.set_mode ps `Math)
let begin_text ps = begin
  if current_mode ps = `Math then begin
    open_node_list ps `HBox;
...
let end_text ps = begin
  let nodes = close_node_list ps `HBox in
  match current_mode ps with
  | `Preamble | `Galley | `Paragraph -> begin
      Mode.ensure_par_mode ps;
...
let right_delim ps = begin
  ...
  let nodes =
    List.map
      (fun x -> match x with
       | Node.Nodes ns -> ns
       | _             -> begin log_warn (location ps) "syntax error in left-right statement!"; [] end
       )
      (close_node_list ps `Math) in
  add_node ps (Node.LeftRight (location ps, nodes))
end
let fraction ps = begin
...
let format_counter ps fmt ctr = begin
  ...
  let select n str = begin
    let str_stream = UCStream.of_list str in

    let rec iter arg i = begin
      if i <= 0 then
        arg
      else
...
let get_counter ps = begin
  let fmt = arg_expanded ps in
  let ctr = Array.of_list (arg_expanded ps) in
  UCStream.insert_list ps.input_stream (format_counter ps fmt ctr)
end
let expand_get_counter ps _ = begin
  let fmt = arg_expanded ps in
  let ctr = Array.of_list (arg_expanded ps) in
  let val_list = format_counter ps fmt ctr in
  val_list @ Macro.expand ps
end
(* layout *)

let shipout_pages ps = begin
...
let new_area ps = begin
  ...
  if area_type = str_galley then begin
  ...
  else if area_type = str_float then begin
    let param = arg_key_val ps in
    add_node ps
      (Node.NewArea (location ps,
         name,
  ...
  else if area_type = str_footnote then begin
    let param = arg_key_val ps in
    add_node ps
      (Node.NewArea (location ps,
         name,
  ...
  else if area_type = str_direct then begin
...
let ensure_vskip ps = begin
  let skip = arg_skip ps in
  let f env glue = begin
...
let nobreak_space ps = begin
  match current_mode ps with
  | `Paragraph | `HBox
  | `LRBox | `RLBox    -> add_node ps (Node.Space (location ps))
  | _                  -> ()
end
let break_space ps = begin
  begin match current_mode ps with
  | `Paragraph | `HBox
  | `LRBox | `RLBox    -> begin
...
let expand_break_space ps _ = begin
  (32 :: Macro.expand ps)
end
let newline ps = begin
  begin match current_mode ps with
  | `Paragraph | `HBox
  | `LRBox | `RLBox    -> begin
      add_node ps (Node.Break (location ps, None, false, [], [], []));
...
let expand_newline ps _ = begin
  (10 :: Macro.expand ps)
end
let literal ps = begin
  let rec read_argument i = begin
    if UCStream.get_char ps.input_stream i = (-1) then
      []
    else if Parser.match_substring ps.input_stream (UString.of_ascii "\\endliteral") i then begin
...
let al_declarations ps = begin
  let rec read_argument i = begin
    if UCStream.get_char ps.input_stream i < 0 then
      []
    else if Parser.match_substring ps.input_stream (UString.of_ascii "\\endALdeclarations") i then begin
      let arg = UCStream.take ps.input_stream i in
...
let initialise ps = begin
  let def_expandable_cmd name cmd exp = begin
  ...
  let def_macro_arg name body = begin
  ...
  let def_math_char name code family char = begin
    def_unexpandable_cmd name
      (fun ps -> add_math_char ps (code, (family, family), (char, char)))
  end in
  let def_math_delim name code small_family small_char large_family large_char = begin
    def_unexpandable_cmd name
      (fun ps -> add_math_char ps (code, (small_family, large_family), (small_char, large_char)))
  end in
  let def_math_acc name family char = begin
    def_unexpandable_cmd name
